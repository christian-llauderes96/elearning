...
<div id="accordion">
	...
	$qry = $conn->query("SELECT l.*,s.subject_code,s.description FROM lessons l inner join subjects s on s.id = l.subject_id where l.academic_year_id = '{$academic_year_id}' and l.id in (SELECT lesson_id from lesson_class where class_id = '{$class_id}') ");
	$nums = 0;
